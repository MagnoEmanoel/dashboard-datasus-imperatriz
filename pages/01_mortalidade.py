...
st.set_page_config(page_title="Mortalidade Evitável | DATASUS", layout="wide")
...
# titulo
st.title("Mortalidade Evitável")
st.caption(f"Fonte: SIM (Sistema de Informação sobre Mortalidade) / DATASUS | Banco de dados: {db_type.upper()}")
st.markdown("---")
...
grupo_análise = st.sidebar.selectbox("Selecione o grupo etário", ["Ambos (Resumo)", "Infantil (< 5 anos)", "Jovem e Adulto (5 a 74 anos)"])

if grupo_análise == "Ambos (Resumo)":
    st.subheader("Visão Geral - Mortalidade Evitável em Imperatriz MA")
    ...
    with col1:
        st.markdown(
            ...
            <div class="metric-card" style="border-left-color: {colors['mortalidade_infantil']};">
                <div class="metric-title">Óbitos Totais Analisados</div>
                <div class="metric-value">{int(total_ad + total_inf):,}</div>
                <div class="metric-subtitle">Mortalidade Evitável Acumulada (2020-2024)</div>
            </div>
            """,
    ...
    with col2:
        st.markdown(
            ...
            <div class="metric-card" style="border-left-color: {colors['mortalidade_adulto']};">
                <div class="metric-title">Óbitos Infantil (<5 anos)</div>
                <div class="metric-value">{int(total_inf):,}</div>
                <div class="metric-subtitle">Representa {total_inf / (total_ad + total_inf) * 100:.1f}% do total de óbitos</div>
            </div>
            """,
            unsafe_allow_html=True
        )
    with col3:
        st.markdown(
            ...
            <div class="metric-card" style="border-left-color: {colors['nascimentos_amber']};">
                <div class="metric-title">Óbitos Jovem/Adulto (5-74)</div>
                <div class="metric-value">{int(total_ad):,}</div>
                <div class="metric-subtitle">Representa {total_ad / (total_ad + total_inf) * 100:.1f}% do total de óbitos</div>
            </div>
            """,
            unsafe_allow_html=True
        )
        
    st.markdown("### Comparação Temporal de Tendências")
    
    # junta os dados de ambos os grupos por ano
    óbitos_ad_ano = df_ad[anos].sum()
    óbitos_inf_ano = df_inf[anos].sum()
    
    df_tendencia = pd.DataFrame({
        "Ano": anos,
        "Jovem/Adulto (5 a 74 anos)": óbitos_ad_ano.values,
        "Infantil (< 5 anos)": óbitos_inf_ano.values,
        "Total": (óbitos_ad_ano + óbitos_inf_ano).values
    })
    ...
    fig = px.line(
        ...
        title="Evolução de Óbitos Evitáveis por Grupo Etario",
        markers=True,
        color_discrete_map={
    ...
    aplicar_estilo_layout(fig, title="Evolução de Óbitos Evitáveis por Grupo Etario", x_title="Ano", y_title="Quantidade de Óbitos")
    st.plotly_chart(fig, use_container_width=True)
    
    st.markdown("### Detalhes das Series Temporais")
    st.dataframe(df_tendencia, use_container_width=True, hide_index=True)

elif grupo_análise == "Infantil (< 5 anos)":
    st.subheader("Indicadores de Mortalidade Infantil Evitável (< 5 anos)")
    ...
    with col1:
        st.markdown(
            f"""
            <div class="metric-card" style="border-left-color: {colors['mortalidade_infantil']};">
                <div class="metric-title">Óbitos Infantil Acumulado</div>
                <div class="metric-value">{int(total_inf)}</div>
                <div class="metric-subtitle">Total de óbitos evitaveis menores de 5 anos (2020-2024)</div>
            </div>
            """,
            unsafe_allow_html=True
        )
    with col2:
        st.markdown(
            ...
            <div class="metric-card" style="border-left-color: {colors['nascimentos_emerald']};">
                <div class="metric-title">Média de Óbitos/Ano</div>
                <div class="metric-value">{media_anual:.1f}</div>
                <div class="metric-subtitle">Média anual no periodo analisado</div>
            </div>
            """,
            unsafe_allow_html=True
        )
        
    st.markdown("### Evolução da Mortalidade Infantil Evitável")
    ...
    fig_inf = px.bar(
        ...
        x="Ano",
        y="Óbitos",
        title="Número de Óbitos Evitáveis (<5 anos) por Ano",
        text="Óbitos",
        color_discrete_sequence=[colors["mortalidade_infantil"]]
    )
    fig_inf.update_traces(textposition='inside')
    aplicar_estilo_layout(fig_inf, title="Evolução de Óbitos Evitáveis (<5 anos) por Ano", x_title="Ano", y_title="Óbitos")
    st.plotly_chart(fig_inf, use_container_width=True)
    
    st.markdown("""
> **Nota Metodologica**: A mortalidade em menores de 5 anos é um importante indicador de desenvolvimento social e qualidade da 
> Atenção Básica de saude. Casos evitaveis geralmente estão relacionados a deficiências no acompanhamento 
> pré-natal, assistência ao parto e puerpério e cobertura vacinal na primeira infância.
    """)

elif grupo_análise == "Jovem e Adulto (5 a 74 anos)":
    st.subheader("Análise Detalhada - Mortalidade Jovem e Adulto (5 a 74 anos)")
    ...
    st.markdown(
        ...
        <div class="metric-card" style="border-left-color: {colors['mortalidade_adulto']}; margin-bottom: 20px;">
            <div class="metric-title">Óbitos Evitáveis Acumulados (5 a 74 anos)</div>
            <div class="metric-value">{int(total_ad):,}</div>
    ...
    tab1, tab2 = st.tabs(["Grandes Grupos de Causa", "Causas Específicas (CID)"])
    
    with tab1:
        st.markdown("### Óbitos por Grandes Grupos de Causa Evitável")
        ...
        if len(df_principais) > 0:
            ...
            fig_grupos = px.bar(
                ...
                y="causa_limpa",
                orientation="h",
                title="Total de Óbitos por Grupo de Causa (2020-2024)",
                color_discrete_sequence=[colors["mortalidade_adulto"]],
                labels={"causa_limpa": "Grupo de Causa", "Total_Acumulado": "Óbitos Acumulados"}
            )
            aplicar_estilo_layout(fig_grupos, title="Ranking dos Grupos de Causa Evitável", x_title="Óbitos", y_title="")
            ...
            fig_lin_grupos = px.line(
                ...
                y="Óbitos",
                color="causa_limpa",
                title="Evolução Temporal dos Óbitos por Grupo de Causa",
                markers=True,
                color_discrete_sequence=colors["categorical"]
            )
            aplicar_estilo_layout(fig_lin_grupos, title="Tendência dos Grupos de Causas Evitáveis", x_title="Ano", y_title="Óbitos")
    ...
    with tab2:
        st.markdown("### Principais Causas Específicas de Óbitos Evitáveis")
        ...
        fig_top = px.bar(
            ...
            y="causa_limpa",
            orientation="h",
            title="Top 15 Causas Específicas de Mortes Evitáveis (2020-2024)",
            color_discrete_sequence=[colors["mortalidade_adulto"]],
            labels={"causa_limpa": "Causa Específica", "Total_Acumulado": "Óbitos"}
        )
        aplicar_estilo_layout(fig_top, title="Top 15 Causas Específicas de Mortes Evitáveis", x_title="Óbitos Acumulados", y_title="")
        st.plotly_chart(fig_top, use_container_width=True)
        
        st.markdown("#### Detalhar Tendência de uma Causa Específica")
        causa_select = st.selectbox("Selecione a Causa Específica para análise", df_subgrupos["causa_limpa"].unique())
        ...
        if len(df_causa_sel) > 0:
            ...
            fig_causa_sel = px.line(
                ...
                x="Ano",
                y="Óbitos",
                title=f"Evolução Temporal: {causa_select}",
                markers=True,
                color_discrete_sequence=[colors["mortalidade_adulto"]]
            )
            aplicar_estilo_layout(fig_causa_sel, title=f"Histórico de Óbitos: {causa_select}", x_title="Ano", y_title="Óbitos")
